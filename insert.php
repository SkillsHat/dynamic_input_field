...
if(isset($_POST["quiz_question"])) {
   ...
   $query = '';
    ...
    for($count = 0; $count<count($quiz_question); $count++) {
        ...
        if($quiz_question_clean != '' && $quiz_opt1_clean != '' && 
           $quiz_opt2_clean != '' && $quiz_opt3_clean != '' && $quiz_opt4_clean != '' && $quiz_c_ans_clean != '' && $quiz_explan_clean != '') {
    ...
    if($query != '') {
        if($connect->multi_query($query)) {
          $last = $connect->insert_id;
            echo $last . 'Item Data Inserted';
        } else {
            echo 'Error';
        }
    } else {
